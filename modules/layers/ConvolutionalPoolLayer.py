...
class ConvolutionalPoolLayer(Layer):
    def __init__(self, image_shape, filter_shape, pool_shape, activation=ReLU, weights_initializer=None,
                                                                               biases_initializer=None):
        super(ConvolutionalPoolLayer, self).__init__(weights_initializer, biases_initializer)
    ...
    def init_weights(self):
        ...
        weights.tag.initializer = Normal(std=np.sqrt(1.0/(np.prod(self.filter_shape)/self.filter_shape[1])))
        return weights
